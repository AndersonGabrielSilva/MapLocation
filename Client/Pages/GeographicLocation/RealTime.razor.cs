...
namespace BlazorGPS.Client.Pages.GeographicLocation
{
    public class RealTimeBase : Base, IAsyncDisposable
    {
            ...
            if (firstRender)
            {
                try
                {
                    if (firstRender)
                    {
                        ...
                        //Metodo que pode ser invocado do servidor
                        hubConnection.On<string>("DanielPolo", (mensagem) =>
                         {
        ...
        protected void SendLocation(GeolocationPosition Position)
        {
            ...
            //Como invocar um metodo que está lá dentro do servidor
